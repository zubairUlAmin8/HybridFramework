package mawad.path_repo;

public class LandingPagePR {
    public final static String dashboardTab="input[type=\"text\"]";
    public final static String LagislationLibraryTab="#legislationView";
    public final static String loaderMain = "div[class=\"absolute inset-0 w-full z-[1111] bg-base-white/60 flex items-center justify-center\"]";
    public final static String profileIcon = "#headlessui-menu-button-5";
    public final static String signOutBtn = "button[class*=\"sign-out-btn\"]";
    public final static String organizationSettingTab = "#organizationSettings";
    public final static String legislationLibraryTab = "#legislationView";
    public final static String riskRegisterTab = "#risksView";
    public final static String actionCenterTab = "#actionsView";
    public final static String reportTab = "#reportsView";
    public final static String organizationDetailsST = "#reportsView";
    public final static String departmentsST = "#settings-1";
    public final static String locationST = "#reportsView";
    public final static String jobFunctionST = "#reportsView";
    public final static String userSetupST = "#reportsView";
    public final static String manageSubscriptionST = "#reportsView";
    public final static String riskMatrixST = "#reportsView";

}
